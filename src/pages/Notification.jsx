 ...
 * 
 * This component is now connected to the notification store:
 * 
 * 1. INTEGRATED: Notification store for state management
 * 2. MAINTAINED: All existing design and functionality
 * 3. PREPARED: Backend notification integration
 * 
 * BACKEND INTEGRATION APPROACH:
 * - Use WebSocket connection for real-time notifications
 * - Call addNotification() when backend sends data
...
const Notification = () => {
  ...
  // Initialize with sample data if notifications are empty
  // useEffect(() => {
  ...
  // Add each notification using the store
  //   sampleNotifications.forEach(notification => {
  //     addNotification(notification);
  //   });
  // };

  // Function to simulate receiving notifications from backend (now uses store)
  ...
  //   addNotification(backendNotification);
  // };
  ...
  const getPriorityColor = (priority) => {
    // Provide default value if priority is undefined
  ...
  const urgentCount = notifications.filter(n => (n.priority || 'normal') === 'urgent').length;
            ...
            {filteredNotifications.length === 0 ? (
              ...
              <div className="space-y-4">
                {filteredNotifications.map((notification) => (
                  <div
                    key={notification.id}
                    className={`bg-gradient-to-br from-slate-800/80 to-slate-700/80 rounded-2xl p-6 border border-slate-600/30 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 backdrop-blur-sm ${!notification.read ? 'ring-2 ring-cyan-400/30' : ''
                      }`}
                  >
